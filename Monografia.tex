...
compartilha vínculo de controle efetivo — BTG e Pan — apresenta o maior
coeficiente da tabela (+5,95 pontos percentuais, $p < 0{,}0001$), e o par
...
modelo de negócio — apresenta o único coeficiente negativo (--2,17 pontos
percentuais, $p = 0{,}014$). O padrão contraria uma hipótese puramente
categórica: compartilhar um rótulo de classificação não produz, por si só,
conectividade adicional, e pode até estar associado a menos.

A leitura não se resume, porém, a "vínculo societário organiza a rede,
categoria nominal não". O par Itaú e Bradesco — os dois maiores bancos
privados nacionais da amostra, sem qualquer vínculo de controle entre si —
também apresenta coeficiente elevado (+5,81 pontos percentuais,
$p < 0{,}0001$), estatisticamente indistinguível do par BTG--Pan. Como a
regressão já controla separadamente pelo efeito de ambos serem instituições
de grande porte (+4,14 pontos percentuais, $p < 0{,}0001$), o coeficiente de
...
risco, nem pelo porte. Duas explicações são compatíveis com os dados: a
primeira é o vínculo de controle societário testado nesta subseção; a segunda
é a proximidade de modelo de negócio e de exposição a choques regulatórios e
...
\midrule
Bradesco e Itaú (privados nacionais)        & 5,81 &  8,13 & $<0{,}0001$ \\
BTG e Pan (vínculo societário)              & 5,95 &  7,46 & $<0{,}0001$ \\
Banco do Brasil e Banrisul (estatais)       & 0,79 &  1,65 & 0,108 \\
ABC Brasil e Santander ("estrangeiros")     & --2,17 & --2,59 & 0,014 \\
Ambos de grande porte                       & 4,14 &  5,51 & $<0{,}0001$ \\
\bottomrule
\end{tabular}
\legend{Fonte: elaboração própria. Erros-padrão clusterizados por par não-dirigido.
Todas as regressões incluem efeitos fixos de emissor e de receptor.}
\end{table}
...
totalizando 84 cortes ao longo do painel de extensão, e resume a série de
...
positivo em 77 dos 84 cortes (91,7\%), com média de 5,32 pontos percentuais
($t = 13{,}30$); o par Itaú--Bradesco e a indicadora de porte mantêm sinal
positivo em todos os 84 cortes; o par ABC Brasil--Santander mantém sinal
negativo na grande maioria deles (apenas 19 de 84 positivos). A evidência
não repousa, portanto, sobre o resultado pontual de uma única estimação, mas
sobre a repetição do padrão de sinal ao longo de sete anos de dados mensais.
...
\midrule
Bradesco e Itaú (privados nacionais)        & 6,21 & 84/84 & 19,42 & $<0{,}0001$ \\
BTG e Pan (vínculo societário)              & 5,32 & 77/84 & 13,30 & $<0{,}0001$ \\
Banco do Brasil e Banrisul (estatais)       & 0,81 & 51/84 &  3,81 & 0,0003 \\
ABC Brasil e Santander ("estrangeiros")     & --1,17 & 19/84 & --5,92 & $<0{,}0001$ \\
Ambos de grande porte                       & 3,60 & 84/84 & 23,19 & $<0{,}0001$ \\
\bottomrule
\end{tabular}
\legend{Fonte: elaboração própria. Teste $t$ da média dos 84 coeficientes mensais contra zero.}
\end{table}
...
máximo em 2022 (9,71 pontos percentuais em média anual), um ano após o BTG
...
coeficiente cai de forma contínua até 2024 (1,24 ponto percentual, o mínimo da
série), voltando a subir em 2025 (2,47 pontos percentuais) — movimento que
...
respectivamente — consistente com o mecanismo de arbitragem esperado quando a
...
Segundo, e mais importante, confirma o mecanismo por trás do resultado da
subseção: se o vínculo de controle societário aproxima o comportamento de
duas ações, o vínculo mais forte possível — a fusão completa — deveria
aproximá-las ao limite. Foi exatamente o que se observou. O mesmo fato que
obriga o encerramento do painel de extensão é o que confirma, ao final do
processo, o resultado que a subseção buscou estabelecer com efeitos fixos e
dados mensais ao longo de sete anos.
...
incorporação — não porque a amostra foi delimitada para excluí-lo. Em
contrapartida, é o painel principal que permite alcançar o regime de
conectividade elevada de 2026 (Subseção~\ref{sec:amostra}), fora do alcance
do painel de extensão.
...
A volatilidade Rogers-Satchell é estimada conforme a
equação~\eqref{eq:rogers-satchell}. Aplica-se
